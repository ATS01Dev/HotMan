...
@Component({
    selector: 'jhi-fonction-hotpersonel',
    templateUrl: './fonction-hotpersonel.component.html'
})
export class FonctionHotpersonelComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/fonction-hotpersonel'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/fonction-hotpersonel', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: FonctionHotpersonel) {
        return item.id;
    }
