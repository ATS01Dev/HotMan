...
@Component({
    selector: 'jhi-personel-hotpersonel',
    templateUrl: './personel-hotpersonel.component.html'
})
export class PersonelHotpersonelComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/personel-hotpersonel'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/personel-hotpersonel', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: PersonelHotpersonel) {
        return item.id;
    }
