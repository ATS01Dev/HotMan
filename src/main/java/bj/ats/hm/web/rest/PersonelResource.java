...
public class PersonelResource {
    ...
    public ResponseEntity<Personel> createPersonel(@Valid @RequestBody Personel personel) throws URISyntaxException {
        ...
        if (personel.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new personel cannot already have an ID")).body(null);
        }
    ...
    public ResponseEntity<Personel> updatePersonel(@Valid @RequestBody Personel personel) throws URISyntaxException {
        ...
        if (personel.getId() == null) {
            return createPersonel(personel);
        }
        Personel result = personelService.save(personel);
    ...
    @Timed
    public ResponseEntity<List<Personel>> getAllPersonels(@ApiParam Pageable pageable) {
    ...
    public ResponseEntity<Personel> getPersonel(@PathVariable Long id) {
        ...
        Personel personel = personelService.findOne(id);
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(personel));
    }
    ...
    /**
     * SEARCH  /_search/personels?query=:query : search for the personel corresponding
     * to the query.
     *
    ...
    @Timed
    public ResponseEntity<List<Personel>> searchPersonels(@RequestParam String query, @ApiParam Pageable pageable) {
        log.debug("REST request to search for a page of Personels for query {}", query);
        Page<Personel> page = personelService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/personels");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
